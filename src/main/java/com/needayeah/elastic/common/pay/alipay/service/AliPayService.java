package com.needayeah.elastic.common.pay.alipay.service;
...
import com.needayeah.elastic.common.pay.alipay.bean.AlipayBean;
import com.needayeah.elastic.common.pay.alipay.config.AlipayConfig;
 ...
 */
@Service("AliPay")
public class AliPayService implements IPayment {
    ...
    public PayResultResponse pay(Order order) {
        ...
        alipayBean.setSubject(order.getGoodsName());
        alipayBean.setTotal_amount(String.valueOf(order.getTotalAmount().doubleValue()));
        alipayBean.setBody(order.getInfo());
        // 1、获得初始化的AlipayClient
        AlipayClient alipayClient = new DefaultAlipayClient(AlipayConfig.GATEWAY_URL, AlipayConfig.APP_ID, AlipayConfig.PRIVARY_KEY, AlipayConfig.FORMAT, AlipayConfig.CHARSET, AlipayConfig.PUBLIC_KEY, AlipayConfig.SIGN_TYPE);
        // 2、设置请求参数
        AlipayTradePagePayRequest alipayRequest = new AlipayTradePagePayRequest();
